...
// waiting secret
constexpr uint64_t SECRET_A_SIZE = 1024 * 1024 * 1024;    // 单个密钥A的大小为1GB 1073741824
constexpr uint16_t SAVE_INDEX_LEN = 16;            // save index长度

// running secret
constexpr uint S_LEN = 1024;                      // 每次传输的密文Bytes
constexpr uint CRC_LEN = 10;                      // 2^32 最长为10位
constexpr uint BIT = 8;
constexpr uint64_t M = S_LEN * BIT;               // 1024 * 8 = 8192
constexpr uint64_t N = M;
constexpr uint16_t Q = 97;
constexpr uint16_t EACH_NUM = 1024;               // 每次发送1024个S_LEN长的秘钥

// generate secret
constexpr uint16_t THREAD_NUM = 2;
constexpr uint16_t FILE_NUM = 4;
constexpr uint16_t READ_LEN = 128;                 // 每次取128个bit 异或得到1bit
constexpr uint16_t READ_NUM = 1024;               // 取1024次 相当于可得到128bit密钥
constexpr uint64_t SECRET_FILE_SIZE = M * N * sizeof(S_Type);  // 暂定uint16
constexpr uint64_t RANDOM_SECRET = 2863311530;
...
constexpr char * KEY_SECRET_INDEX_TIME_ZSET = "shannonnet_%s_save_index_time_zset";     // 保存生成完毕的秘钥 index => create_time
constexpr char * KEY_SECRET_INDEX_COUNT_ZSET = "shannonnet_count_index_num_zset";    // 秘钥和读取次数 index => num 
constexpr char * KEY_NODE_INDEX_VALID_ZSET = "shannonnet_node_%d_%d_index_valid_zset";  // 每个节点下的所有有效的秘钥文件 index => valid: 1 valid; 2 invalid
